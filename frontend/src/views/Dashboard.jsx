...
export default function Dashboard({ user, logs = [], dataStatus = 'ready', dataError = null }) {
  ...
  // --- CALCULS POUR LE CAMEMBERT DYNAMIQUE ---
  const totalThreats = criticalCount + warningCount + resolvedCount;
  const pctCritical = totalThreats > 0 ? Math.round((criticalCount / totalThreats) * 100) : 0;
  const pctWarning = totalThreats > 0 ? Math.round((warningCount / totalThreats) * 100) : 0;
        ...
        <Card className="flex flex-col justify-between">
          <div>
            ...
            <div className="flex justify-center my-5">
              <div 
                style={{
                  background: totalThreats > 0 ? `conic-gradient(
                    #ef4444 0% ${pctCritical}%, 
                    #f59e0b ${pctCritical}% ${pctCritical + pctWarning}%, 
                    #10b981 ${pctCritical + pctWarning}% 100%
                  )` : 'var(--surface-3)'
                }}
                className="w-32 h-32 rounded-full flex items-center justify-center relative shadow-sm"
              >
                {/* Centre évidé du donut qui s'adapte au thème */}
                <div className="w-22 h-22 rounded-full flex flex-col items-center justify-center" style={{ background: 'var(--surface-1)' }}>
                  <span className="text-[10px] uppercase font-bold" style={{ color: 'var(--text-muted)' }}>Total</span>
                  <span className="text-base font-bold" style={{ color: 'var(--text-primary)' }}>{totalThreats}</span>
                </div>
              </div>
            </div>
          </div>

          {/* --- LISTE DESCRIPTIVE EXISTANTE --- */}
          <div className="space-y-3">
            {[
              { label: 'Critique', value: criticalCount, Icon: ShieldAlert },
              { label: 'Avertissement', value: warningCount, Icon: AlertTriangle },
              { label: 'Résolues', value: resolvedCount, Icon: CheckCircle2 },

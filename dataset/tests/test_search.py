...
class TestSearchBySourceIP:
    ...
    def test_cidr_range_10_0_0_0_8(self):
        # Vérifie que le filtre CIDR ne retourne que des IPs dans la plage,
        # quel que soit le nombre de documents dans l'index.
    ...
    def test_cidr_no_match_returns_empty(self):
        # Utilise une plage réservée IANA (TEST-NET, RFC 5737) garantie absente
        # des données générées par Faker (qui produit des IPs publiques aléatoires).
        result = search_logs(source_ip="203.0.113.0/24")
        assert result["total"] == 0
